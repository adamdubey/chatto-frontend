...
resource "aws_cloudfront_distribution" "chatapp_cloudfront_distribution" {
  ...
  origin {
    domain_name = aws_s3_bucket.chattoapp_s3_bucket.bucket_regional_domain_name
    origin_id   = aws_s3_bucket.chattoapp_s3_bucket.id
    s3_origin_config {
  ...
  default_cache_behavior {
    ...
    target_origin_id       = aws_s3_bucket.chattoapp_s3_bucket.id
